...
bool CurlListdirOp::ParseProp(DavEntry &entry, tinyxml2::XMLElement *prop)
{
    for (auto child = prop->FirstChildElement(); child != nullptr; child = child->NextSiblingElement()) {
        if (!strcasecmp(child->Name(), "D:resourcetype") || !strcasecmp(child->Name(), "lp1:resourcetype")) {
            auto collection = child->FirstChildElement("D:collection");
            entry.m_isdir = collection != nullptr;
            if (entry.m_isdir && entry.m_size < 0) {
                entry.m_size = 0;
            }
        } else if (!strcasecmp(child->Name(), "D:getcontentlength") || !strcasecmp(child->Name(), "lp1:getcontentlength")) {
        ...
        } else if (!strcasecmp(child->Name(), "D:getlastmodified") || !strcasecmp(child->Name(), "lp1:getlastmodified")) {
        ...
        } else if (strcasecmp(child->Name(), "D:href") == 0) {
            auto href = child->GetText();
            if (href == nullptr) {
                return false;
            }
            entry.m_name = href;
        } else if (!strcasecmp(child->Name(), "D:executable") || !strcasecmp(child->Name(), "lp1:executable")) {
...
CurlListdirOp::ParseResponse(tinyxml2::XMLElement *response)
{
    ...
    for (auto child = response->FirstChildElement(); child != nullptr; child = child->NextSiblingElement()) {
        if (!strcasecmp(child->Name(), "D:href")) {
            auto href = child->GetText();
            if (href == nullptr) {
                return {entry, false};
            }
        ...
        for (auto propstat = child->FirstChildElement(); propstat != nullptr; propstat = propstat->NextSiblingElement()) {
            if (strcasecmp(propstat->Name(), "D:prop")) {
                continue;
            }
